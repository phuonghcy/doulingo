...
namespace Duolingo_1
{
    ...
    public partial class TuMoiPage : ContentPage
    {
        ...
        async void LayDsTuDN(tumoi t)
        {
            ...
            var chuoi = await http.GetStringAsync("http://192.168.137.67/webapi/api/ServiceController/LayDsTuDongNghia?matu=" + t.matu.ToString());
        ...
        {
            HttpClient http = new HttpClient();
            var chuoi = await http.GetStringAsync("http://192.168.137.67/webapi/api/ServiceController/LayDsTuTraiNghia?matu=" + t.matu.ToString());
